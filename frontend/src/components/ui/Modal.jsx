  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end justify-center p-0 sm:items-center sm:p-6">
      ...
      <div
        ...
        className={[
          'relative z-10 mx-4 flex max-h-[92svh] shrink-0 flex-col rounded-t-3xl border border-[#e8ecf1] bg-white shadow-[0_24px_60px_rgba(15,23,42,0.18)] sm:mx-0 sm:rounded-[1.25rem]',
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-[#eef2f0] px-5 py-4 sm:px-6">
          ...
          <button
            type="button"
            onClick={onClose}
            className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-[#6b7280] hover:bg-[#f4f6fb] hover:text-[#111827]"
            aria-label="Close"
          >
            <X className="h-4 w-4" />
          </button>
        ...
        {footer ? (
          <div className="flex flex-wrap items-center justify-end gap-2 border-t border-[#eef2f0] px-5 py-4 sm:px-6">
            {footer}
          </div>
        ) : null}
      </div>

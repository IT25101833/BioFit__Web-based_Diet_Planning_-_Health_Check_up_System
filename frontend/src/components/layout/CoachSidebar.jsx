...
const navItems = [
  ...
  { label: 'My Clients', to: '/coach/clients', icon: Users },
  { label: 'Exercise Library', to: '/coach/exercises', icon: Library },
...
export default function CoachSidebar({ mobileOpen, onClose }) {
  const { user } = useAuth()
  ...
  return (
    ...
    >
      <div className="flex h-[72px] shrink-0 items-center gap-2.5 border-b border-[#e8ecf1] px-5">
        <Link to="/coach/dashboard" className="inline-flex items-center gap-2.5" onClick={onClose}>
      ...
      <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto px-3 py-5" aria-label="Fitness Coach">
      ...
      <div className="mt-auto shrink-0 border-t border-[#e8ecf1] bg-white p-4">
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[11px] text-[#6b7280]">Fitness Coach</p>
          </div>
        </Link>

...
export default function SupportTicketDetails() {
  ...
  async function handleAssign(officerName) {
    const updated = await assignSupportTicket(ticket.id, officerName)
    setTicket(updated)
    setToast(`Ticket ${ticket.id} assigned to ${officerName}.`)
  }

  async function handleAssignToMe() {
    const updated = await assignSupportTicket(ticket.id, 'Priya Nair')
    setTicket(updated)
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px] xl:grid-cols-[1fr_380px]">
        ...
        <div className="space-y-6">
          ...
          {ticket.relatedService ? (
            <RelatedBioFitService service={ticket.relatedService} />
          ) : null}

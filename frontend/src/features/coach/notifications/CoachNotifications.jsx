...
} from './data/coachNotificationData'
...
export default function CoachNotifications() {
  ...
  async function load() {
    ...
    setError('')
    try {
      setItems(await fetchCoachNotifications())
    } catch {
  ...
  return (
    <div>
      <PageHeader
        title="Notifications"
        description="Stay updated on clients, plans, assessments and schedule changes."
        actions={
          unreadCount > 0 ? (
            <Button
              variant="outline"
              ...
              onClick={async () => {
                await markAllCoachNotificationsRead()
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description="New coaching updates will appear here."
        />
      ) : (
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                {!item.read ? (
                  <Button
                    ...
                    className="!border-[#005a40]/25 !text-[#005a40]"
                    onClick={async () => {
                      await markCoachNotificationRead(item.id)
                      setItems((prev) =>

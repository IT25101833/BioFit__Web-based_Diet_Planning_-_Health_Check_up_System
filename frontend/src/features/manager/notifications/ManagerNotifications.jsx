...
} from './data/managerNotificationData'
...
export default function ManagerNotifications() {
  ...
  async function load() {
    ...
    setError('')
    try {
      setItems(await fetchManagerNotifications())
    } catch {
  ...
  if (error) {
    return <ErrorState title="We couldn’t load notifications." onRetry={load} />
  }
  ...
  return (
    <div>
      <PageHeader
        title="Notifications"
        description="Operational updates across programmes, scheduling, and appointments."
        actions={
          unreadCount > 0 ? (
            <Button
              variant="outline"
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description="New operational updates will appear here."
        />
      ) : (
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                {!item.read ? (
                  <Button
                    ...
                    className="!border-[#005a40]/25 !text-[#005a40]"
                    onClick={async () => {
                      await markManagerNotificationRead(item.id)
                      setItems((prev) =>
